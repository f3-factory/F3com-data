...
Example with a [Google Static Maps object](google-static-maps-v2 "Google Static Maps API v2 plug-in"):

```php
$map = new \Web\Google\StaticMap();
echo $f3->stringify($map);
// Outputs e.g.:
'Web\\Google\\StaticMap::__set_state('query'=>array(array('center','Mont Fuji'),array('size','320x240'),array('zoom','12'),array('maptype','satellite')))'
```
...
The plural type syntax must starts with `0, plural,` followed by a list of plural keywords associated with the desired output. The accepted plural keywords are '*zero*', '*one*', '*two*' and '*other*'. 
...
Then shutdowns the application and calls the shutdown handler defined in [UNLOAD](quick-reference#unload).
